...
const Tooth2d = ({ userIds }) => {
    ...
    const fetchMedicalHistory = async () => {
        try {
            const response = await axios.get(`${Baseurl}/tooth2dmodel/get-all-data/${userId}`);
            ...
            <div className='grid grid-cols-2'>
                <div className='flex flex-cols'>
                    <button
                        ...
                        className="bg-[#3EB489] hover:bg-[#62A78E] text-white rounded-lg p-2  transition duration-300"
                    >
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900 bg-opacity-75">
                    <div className="bg-[#C6E4DA] p-6 rounded-lg shadow-lg w-full max-w-lg">
                        ...
                        <div className="mb-4 max-h-60 overflow-y-auto">
                            <table className="table-auto w-full text-left border border-black ">
                                <thead>
                                    <tr>
                                        <th className="px-4 py-2 text-center bg-[#3EB489] text-white border border-black">Note</th>
                                        <th className="px-4 py-2 text-center bg-[#3EB489] text-white border border-black">Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {getNotesForTooth(selectedTooth.id).map((note, index) => (
                                        <tr key={`modal-note-${index}`} className="border cursor-pointer">
                                            <td className="px-4 py-2 border border-black bg-gray-100">{note}</td>
                                            <td className="px-4 py-2 border border-black bg-gray-100">
